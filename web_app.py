...
def run_pipeline_thread(run_id, timeframes, raw_data_map, indicator_map, results_map, log_dir, results_dir):
    ...
    try:
        ...
        if pipeline_successful and backtest_status.get("running") and backtest_status.get("run_id") == run_id: # Check status before reporting
            app.logger.info(f"Pipeline loop finished for {run_id}. Generating final HTML report...")
            final_message = f"Run {run_id} completed. Report saved."; report_path = None
            try:
                ...
                report_path.write_text(html_report_content, encoding='utf-8'); app.logger.info(f"HTML report saved to {report_path}")
            except Exception as report_e: app.logger.error(f"Failed to generate/save report: {report_e}", exc_info=True); final_message = f"Run {run_id} completed, report generation failed."
